...
function Profile() {
  ...
  return (
    <div className="flex flex-col gap-10 items-start phone:items-center phone:w-[400px] tablet:w-[500px] desktop:w-5/6">
      <h2 className="phone:w-[350px] text-mainHeadersColor font-semibold phone:text-3xl tablet:text-4xl desktop:text-5xl">
        {data[language].profile.header}
      </h2>
      <div className="flex desktop:gap-32 phone:gap-10 phone:w-[350px] phone:flex-col">
          ...
          {data[language].profile.infoArray.map((info, index) => {
            return (
              <div className="flex justify-between" key={index}>
                <p className="text-userInfosColor font-semibold phone:w-[120px] desktop:w-[300px]">
                  {info.title}:
                </p>
                <p className="text-userInfosColor text-left phone:w-[220px] desktop:w-[400px] ">
                  {info.value}
                </p>
              </div>
            );
